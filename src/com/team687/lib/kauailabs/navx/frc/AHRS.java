...
package com.team687.lib.kauailabs.navx.frc;

import com.team687.lib.kauailabs.navx.AHRSProtocol;
import com.team687.lib.kauailabs.navx.AHRSProtocol.AHRSPosUpdate;
import com.team687.lib.kauailabs.navx.AHRSProtocol.BoardID;
import com.team687.lib.kauailabs.navx.IMUProtocol.YPRUpdate;
...
public class AHRS extends SensorBase implements PIDSource, LiveWindowSendable {
     ...
     * these axes are board-relative ("Board Frame"), and are not necessarily the
     * same as the logical axes of the chassis on which the sensor is mounted.
     * 
	...
	kBoardAxisX(0), kBoardAxisY(1), kBoardAxisZ(2);
    ...
    /**
     * For use with serial communications, the SerialDataType specifies the type of
     * data to be streamed from the sensor. Due to limitations in the streaming
     * bandwidth on some serial interfaces, only a subset of all available data can
     * be streamed.
     * <p>
     * Note that if communicating over I2C/SPI, all available data can be retrieved,
     * so the SerialDataType need only be specified if using serial communications.
     */
     ...
     * update rate with a custom rate which may be from 4 to 200, representing the
     * number of updates per second sent by the sensor.
     * <p>
    ...
    /**
     * Constructs the AHRS class using serial communication, overriding the default
     * update rate with a custom rate which may be from 4 to 200, representing the
     * number of updates per second sent by the sensor.
     * <p>
     ...
     * Serial interface.
     * <p>
     * Note that the serial interfaces can communicate either processed data, or raw
     * data, but not both simultaneously. If simultaneous processed and raw data are
     * needed, use one of the register-based interfaces (SPI or I2C).
     * <p>
     * Note that increasing the update rate may increase the CPU utilization.
     * <p>
     * 
     ...
     * the sensor. Pitch is a measure of rotation around the X Axis.
     * 
    ...
    /**
     * Returns the current roll value (in degrees, from -180 to 180) reported by the
     * sensor. Roll is a measure of rotation around the X Axis.
     * 
     ...
     * sensor. Yaw is a measure of rotation around the Z Axis (which is
    ...
    /**
     * Returns the current tilt-compensated compass heading value (in degrees, from
     * 0 to 360) reported by the sensor.
     * <p>
     * Note that this value is sensed by a magnetometer, which can be affected by
     * nearby magnetic fields (e.g., the magnetic fields generated by nearby
     * motors).
     * <p>
     * Before using this value, ensure that (a) the magnetometer has been calibrated
     * and (b) that a magnetic disturbance is not taking place at the instant when
     * the compass heading was generated.
     * 
    ...
    /**
     * Sets the user-specified yaw offset to the current yaw value reported by the
     * sensor.
     * <p>
     * This user-specified yaw offset is automatically subtracted from subsequent
     * yaw values reported by the getYaw() method.
     */
    public void zeroYaw() {
	if (board_capabilities.isBoardYawResetSupported()) {
	    io.zeroYaw();
     ...
     * gyro/accelerometer calibration. Automatic calibration occurs when the sensor
     * is initially powered on, during which time the sensor should be held still,
     * with the Z-axis pointing up (perpendicular to the earth).
     * <p>
     ...
     * <p>
     * 
     * @return Returns true if the sensor is currently automatically calibrating the
    ...
    /**
     * Indicates whether the sensor is currently connected to the host computer. A
     * connection is considered established whenever communication with the sensor
     * has occurred recently.
     * <p>
     * 
     * @return Returns true if a valid update has been recently received from the
     *         sensor.
     */
    ...
    /**
     * Returns the count in bytes of data received from the sensor. This could can
     * be useful for diagnosing connectivity issues.
     * <p>
     * If the byte count is increasing, but the update count (see getUpdateCount())
    ...
    /**
     * Returns the navX-Model device's currently configured update rate. Note that
     * the update rate that can actually be realized is a value evenly divisible by
     * the navX-Model device's internal motion processor sample clock (200Hz).
     * Therefore, the rate that is returned may be lower than the requested sample
     * rate.
     *
     ...
     * instance, a request for 58 Hertz will result in an actual rate of 66Hz (200 /
     * (200 / 58), using integer math.
     *
    ...
    /**
     * Returns the currently requested update rate. rate. Note that not every update
     * rate can actually be realized, since the actual update rate must be a value
     * evenly divisible by the navX-Model device's internal motion processor sample
     * clock (200Hz).
     * 
     * To determine the actual update rate, use the {@link #getActualUpdateRate()}
     * method.
     * 
    ...
    /**
     * Returns the sensor timestamp corresponding to the last sample retrieved from
     * the sensor. Note that this sensor timestamp is only provided when the
     * Register-based IO methods (SPI, I2C) are used; sensor timestamps are not
     * provided when Serial-based IO methods (TTL UART, USB) are used.
     * 
     ...
     * <p>
     * World linear acceleration refers to raw acceleration data, which has had the
     * gravity component removed, and which has been rotated to the same reference
     * frame as the current yaw value. The resulting value represents the current
     * acceleration in the x-axis of the body (e.g., the robot) on which the sensor
     * is mounted.
     * <p>
     * 
     ...
     * gravity component removed, and which has been rotated to the same reference
     * frame as the current yaw value. The resulting value represents the current
     * acceleration in the Y-axis of the body (e.g., the robot) on which the sensor
     * is mounted.
     * <p>
     * 
     ...
     * gravity component removed, and which has been rotated to the same reference
     * frame as the current yaw value. The resulting value represents the current
     * acceleration in the Z-axis of the body (e.g., the robot) on which the sensor
     * is mounted.
     * <p>
     * 
    ...
    /**
     * Indicates if the sensor is currently detecting motion, based upon the X and
     * Y-axis world linear acceleration values. If the sum of the absolute values of
     * the X and Y axis exceed a "motion threshold", the motion state is indicated.
     * <p>
     * 
     ...
     * Threshold."
     * <p>
     * Yaw Rotation can occur either when the sensor is rotating, or when the sensor
     * is not rotating AND the current gyro calibration is insufficiently calibrated
     * to yield the standard yaw drift rate.
     * <p>
     * 
     * @return Returns true if the sensor is currently detecting motion.
     */
    ...
    /**
     * Returns the current barometric pressure, based upon calibrated readings from
     * the onboard pressure sensor. This value is in units of millibar.
     * <p>
     * NOTE: This value is only valid for a navX Aero. To determine whether this
     * value is valid, see isAltitudeValid().
     * 
     ...
     * barometric pressure sensor, and the currently-configured sea-level barometric
     * pressure [navX Aero only]. This value is in units of meters.
     * <p>
     ...
     * <p>
     * 
     * @return Returns current altitude in meters (as long as the sensor includes an
     *         installed on-board pressure sensor).
     */
     ...
     * sensor installed.
     * <p>
     * If this value is false for a board with an installed pressure sensor, this
     * indicates a malfunction of the onboard pressure sensor.
     * <p>
     * 
     ...
     * <p>
     * The 9-axis heading is the fusion of the yaw angle, the tilt-corrected compass
     * heading, and magnetic disturbance detection. Note that the magnetometer
     * calibration procedure is required in order to achieve valid 9-axis headings.
     * <p>
     ...
     * <p>
     * This function will always return false if the sensor's magnetometer has not
     * yet been calibrated; see isMagnetometerCalibrated().
     * 
     ...
     * <p>
     * Note that if this function does indicate the magnetometer is calibrated, this
     * does not necessarily mean that the calibration quality is sufficient to yield
     * valid compass headings.
     * <p>
     * 
    ...
    /**
     * Returns the imaginary portion (W) of the Orientation Quaternion which fully
     * describes the current sensor orientation with respect to the reference angle
     * defined as the angle at which the yaw was last "zeroed".
     * <p>
     * Each quaternion value (W,X,Y,Z) is expressed as a value ranging from -2 to 2.
     * This total range (4) can be associated with a unit circle, since each circle
     * is comprised of 4 PI Radians.
     * <p>
    ...
    /**
     * Returns the real portion (X axis) of the Orientation Quaternion which fully
     * describes the current sensor orientation with respect to the reference angle
     * defined as the angle at which the yaw was last "zeroed".
     * <p>
     * Each quaternion value (W,X,Y,Z) is expressed as a value ranging from -2 to 2.
     ...
     * previous sample). Finally, this value is converted to displacement in meters,
     * and integrated.
     * 
     * @return none.
     */
     ...
     *
     * NOTE: This feature is experimental. Velocity measures rely on integration of
     * acceleration values from MEMS accelerometers which yield "noisy" values. The
    ...
    /**
     * Registers a callback interface. This interface will be called back when new
     * data is available, based upon a change in the sensor timestamp.
     * <p>
     * Note that this callback will occur within the context of the device IO
     * thread, which is not the same thread context the caller typically executes
     * in.
     */
     ...
     *
     * Be sure to deregister any callback which have been previously registered, to
     * ensure that the object implementing the callback interface does not continue
     ...
     * sensor.
     * <p>
     * NOTE: The angle is continuous, meaning it's range is beyond 360 degrees. This
     * ensures that algorithms that wouldn't want to see a discontinuity in the gyro
     * output as it sweeps past 0 on the second time around.
     * <p>
     * Note that the returned yaw value will be offset by a user-specified offset
     * value; this user-specified offset value is set by invoking the zeroYaw()
     * method.
     ...
     *         degrees. This heading is based on integration of the returned rate
    ...
    /**
     * Sets an amount of angle to be automatically added before returning a angle
     * from the getAngle() method. This allows users of the getAngle() method to
     * logically rotate the sensor by a given amount of degrees.
     * <p>
    ...
    /**
     * Returns the currently configured adjustment angle. See setAngleAdjustment()
     * for more details.
     * 
     ...
     * Reset the Yaw gyro.
     * <p>
     * Resets the Gyro Z (Yaw) axis to a heading of zero. This can be used if there
     * is significant drift in the gyro and it needs to be recalibrated after it has
     * been running.
     */
    public void reset() {
	zeroYaw();
     ...
     * "Pitch". Calibrated and Integrated Pitch data is accessible via the
     * {@link #getPitch()} method.
     * <p>
     * 
     ...
     * "Roll". Calibrated and Integrated Pitch data is accessible via the
     * {@link #getRoll()} method.
     * <p>
     * 
     * @return Returns the current rotation rate (in degrees/sec).
     ...
     * degrees/sec). NOTE: this value is un-processed, and should only be accessed
     * by advanced users. Typically, rotation about the T Axis is referred to as
     * "Yaw". Calibrated and Integrated Pitch data is accessible via the
     * {@link #getYaw()} method.
     * <p>
     * 
     * @return Returns the current rotation rate (in degrees/sec).
     ...
     * This raw value has not had acceleration due to gravity removed from it, and
     * has not been rotated to the world reference frame. Gravity-corrected, world
     * reference frame-corrected X axis acceleration data is accessible via the
     ...
     * This raw value has not had acceleration due to gravity removed from it, and
     * has not been rotated to the world reference frame. Gravity-corrected, world
     * reference frame-corrected Y axis acceleration data is accessible via the
     ...
     * This raw value has not had acceleration due to gravity removed from it, and
     * has not been rotated to the world reference frame. Gravity-corrected, world
     * reference frame-corrected Z axis acceleration data is accessible via the
     ...
     * Tilt-corrected compass heading data is accessible via the
     ...
     * <p>
     * This value may be useful in order to perform advanced temperature- correction
     * of raw gyroscope and accelerometer values.
     * <p>
     * 
     ...
     * direction (up/down) is currently configured to report Yaw (Z) angle values.
     * NOTE: If the board firmware supports Omnimount, the board yaw axis/direction
     * are configurable.
     * <p>
	...
	public void setAHRSPosData(AHRSPosUpdate ahrs_update, long sensor_timestamp) {
	    ...
	    AHRS.this.is_moving = (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0) ? true
		    : false);
	    AHRS.this.is_rotating = (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0)
		    ? false
		    : true);
	    AHRS.this.altitude_valid = (((ahrs_update.sensor_status
		    & AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0) ? true : false);
	    AHRS.this.is_magnetometer_calibrated = (((ahrs_update.cal_status
		    & AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0) ? true : false);
	    AHRS.this.magnetic_disturbance = (((ahrs_update.sensor_status
		    & AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0) ? true : false);
		...
		if (callback != null) {
		    ...
		    callback.timestampedDataReceived(system_timestamp, sensor_timestamp, ahrs_update,
			    callback_contexts[i]);
		}
	    }
	}
	...
	public void setAHRSData(AHRSProtocol.AHRSUpdate ahrs_update, long sensor_timestamp) {
	    ...
	    updateDisplacement(AHRS.this.world_linear_accel_x, AHRS.this.world_linear_accel_y, update_rate_hz,
		    AHRS.this.is_moving);
